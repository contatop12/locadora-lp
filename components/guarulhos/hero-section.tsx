...
export function HeroSection() {
  const benefits = [
    ...
    { icon: Truck, text: "Entrega em Guarulhos" },
  ]
      ...
      <div className="relative z-10 container mx-auto px-4 h-full flex items-center min-h-[calc(100vh-72px)]">
        <div className="grid lg:grid-cols-2 gap-6 lg:gap-10 items-center w-full py-8 md:py-12">
          ...
          <div className="space-y-4 md:space-y-5">
            <h1
              id="hero-heading"
              className="text-3xl md:text-4xl lg:text-5xl font-bold text-background leading-tight text-balance"
              style={{ fontFamily: "var(--font-heading)" }}
            >
              Aluguel de Andaimes em Guarulhos
            </h1>

            <p className="text-base md:text-lg text-background/80 leading-relaxed max-w-lg text-balance">
              Andaimes tubulares de alta qualidade para sua obra em Guarulhos e região metropolitana. 
            ...
            <ul className="flex flex-wrap gap-3 md:gap-4" aria-label="Benefícios">
              {benefits.map((benefit, index) => (
                <li key={index} className="flex items-center gap-2 bg-background/10 backdrop-blur-sm px-3 py-2 rounded-full">
                  <benefit.icon className="w-4 h-4 text-primary" />
                  <span className="text-background text-sm font-medium">{benefit.text}</span>
                </li>
              ))}
            </ul>
          </div>
